...
export function emit(test, options = config, snapshot) {
  return new Promise(async (res, rej) => {
    ...
    if (!options.skipStdLibEmitting) {
      ...
      let _commands = [
        'let times = []; let timeStart = new Date().getTime(); let timeNow; let took;',
      ]

      commands.forEach((c, idx) => {
        let originalCommand = test.commands[idx]
        let commandName = `${originalCommand.command} on ${
          originalCommand.target
        }`
        ...
        _commands.push(c)
        _commands.push(
          `timeNow = new Date().getTime(); took = timeNow - timeStart; times.push({name: '${stringEscape(commandName)}', took, unit: 'millisecond'}); timeStart = timeNow;`
        )
      })

      _commands.push(
        `require('fs').writeFileSync(require('path').join(process.cwd(), '..', '${testName}_execution_time.json'), JSON.stringify(times, null, 4));`
      )
